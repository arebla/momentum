...
\end{equation}
%
onde $V(x)=\frac{k}{2}x^2$ é a enerxía potencial, con $k$ a constante de
...
$p$, e isto dá cero; é dicir, medir $q$ e inmediatamente despois $p$, ou medir
$p$ e inmediatamente despois $q$ é o mesmo. Así, no mundo hamiltoniano clásico,
...
Tras este recordatorio, volvemos á lóxica, aínda que de maneira un tanto
...
porque uns son os puntos reais da recta, e outros os complexos) e, sobre cada
...
ao estado $\ell'\sim|M+1\rangle$ (Olliño! Tamén no punto $x$). O grupo vén a
conto como segue: en $\log(\gamma)$ pasos, pasaremos do estado $|M\rangle$ ao
estado $\left|\frac{N}{2\pi i}\log \exp\left(\frac{2\pi
iM}{N}+{\log(\gamma)}\right)\right\rangle$. A vantaxe desta linguaxe é que non
...
noutro, etc, basta considerar os triplos nas fibras $\pi^{-1}(x)\subset L_N$ e
$\pi^{-1}(x+1)\subset L_N$. Outra forma de velo é que se cortamos (os puntos
reais da) a recta en cachiños $[x,x+1]$, seguimos tendo unha recta dentro. Por
exemplo:
%
\begin{equation}
...
lados: $x$ identifícase co $0$; e $x+1$, co infinito. Obsérvese que isto fai
...
Hai outra construción posible (e equivalente nun sentido adecuado
...
subxacencia de xeometría non conmutativa. O oscilador harmónico é un tanto
