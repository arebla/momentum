...
non ten poder preditivo no réxime no que se está aplicando, e é necesario
...
consistentes dos seus parámetros para conseguir resultados finitos, de xeito
que as diverxencias teóricas se cancelan para non aparecer nos observables.
Este proceso denomínase \textit{regularización}, e existen diversos métodos
...
dun proceso que se denomina \textit{renormalización}, e determina como as altas
...
\textit{naturalidade}, o cal non está exento de debate, aínda a día de hoxe.
...
teoría efectiva válida ata unha escala $\Lambda$, denominada \textit{cutoff},
que determina a escala de enerxía característica empregada no proceso de
renormalización \cite{Polchinski}. Deste xeito, se un operador $O_i$ ten
...
tipos. Se $\delta_i < D$, o operador será \textit{relevante} a baixas enerxías,
...
operador será \textit{marxinal}. Por último, para $\delta_i > D$ o coeficiente
correspondente decrecerá de xeito polinómico en $\Lambda$ e o operador será
\textit{irrelevante} a baixas enerxías. En todo caso, a escala de enerxía
...
O concepto de \textit{naturalidade} introdúcese neste momento: é de esperar que
...
Se isto se cumpre, dise que o parámetro $g_i$ é \textit{natural}. No caso
contrario, se o valor de $c_i$ é moi grande ou moi pequeno, dise que $g_i$ está
\textit{finamente axustado} \cite{Wells}. A primeira impresión que pode xerar
...
Maiani \cite{GIM} e o cálculo da súa masa antes do seu descubrimento
experimental \cite{GaillardLee}.
...
son relevantes a baixas enerxías.
...
onde $c_H$ é adimensional e esperariamos que $\Lambda^2 \sim M^2_p$. Isto, non
obstante, manifesta unha situación incómoda. O valor medido experimentalmente
para a masa é $m_H \approx 126$ GeV, é dicir, $m_H \sim 10^2$ GeV, mentres que
$\Lambda^2 \sim 10^36$ GeV. Isto implica que $c_H \sim \mathcal{O}(10^{-34})$,
o que contradí de xeito espectacular a suposición de que $c_H \sim
\mathcal{O}(1)$.

Este feito ilustra o \textit{problema da xerarquía} do Modelo Estándar: a
...
da masa IR, o que efectivamente nos forza a que $c_H \sim
\mathcal{O}(10^{-34})$.
...
A \textit{supersimetría}, como o seu nome indica, é unha simetría dentro dunha
...
viceversa \cite{SUSY}. Así, esquematicamente, teriamos un campo fermiónico
$\varphi$ relacionado co campo de Higgs $\Phi$ segundo
...
pares $t \bar{t}$ virtuais. O efecto importante da supersimetría é que as
correccións cuánticas inducidas polo novo campo $\varphi$ teñen a mesma forma
que as do quark $t$, pero co signo cambiado. Deste xeito podemos facer que se
cancelen, e así $c_H=0$ automaticamente \cite{Agashe}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{revistas/004/imaxes/naturalness_fig12.png}
    \caption{
        Correccións cuánticas a primeira orde da masa do Higgs no Modelo
        Estándar.
    }
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{
        Correccións cuánticas inducidas polo novo campo fermiónico $\varphi$,
