\Titular%
{divulgacion}%
{A programadora invisible}%
{Manuel Tiago Barbosa Aresta e Victoria Bello Pastor}%
{E como terminou contrariando un Nobel.}%


\begin{multicols}{2}

\Cita{Detrás de cada home hai sempre unha muller poñendo os ollos en
branco.}{Jim Carrey}
...
desenvolvemento científico é recente, e aínda hai moitos teoremas ou logros
científicos atribuídos a unha autoría masculina sen ter en conta as mulleres
que puideron facer diso unha realidade. Unha destas mulleres foi Mary Tsingou,
unha das primeiras programadoras do primeiro supercomputador estadounidense e
partícipe nun dos experimentos de máis impacto no campo da mecánica estatística
e da teoría do caos.

\subsection*{Quen era Mary Tsingou}
...
europeo. No seu retorno ao país da liberdade, Mary realizou os seus estudos de
secundaria, e accedeu a unha posterior formación universitaria. Finalmente, en
1951 graduouse en matemáticas e educación na Universidade de Wisconsin. Nada
máis obter o título, buscou traballo como profesora de matemáticas, mais o
regreso dos veteranos tras a Segunda Guerra Mundial fixo que fose case
imposible encontrar un posto na educación, sobre todo como muller.
...
Neste período, a súa carreira profesional estaba aínda nos seus inicios, pero
ela non podía evitar sentir unha certa frustración cara á

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{revistas/004/imaxes/mary1.png}
    \caption{
        Mary Tsingou na súa xuventude, traballando no laboratorio de Los
        Álamos.
    }
\end{figure}
...
\subsection*{O problema FPU} % Este é sen o ‘T’, todos os demais do texto teñen que ser FPUT
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.83\linewidth]{revistas/004/imaxes/mary2.png}
...
\begin{align}
    \nonumber m\ddot{u}_j =\ &k(u_{j+1} + u_{j-1}-2u_j )\\
    \nonumber &+ 2k\alpha [(u_{j+1} - u_j ) - (u_j - u_{j-1} )]
\end{align}
...
en conta tamén o elemento da non lineal, polo que a ecuación será o
hamiltoniano do oscilador harmónico acoplado máis un termo non lineal:

\begin{align}
    \nonumber H = \frac{1}{2}\sum_{i=0}^N \left(\frac{p_i^2}{m} +
    k(u_{i+1} -u_i)^2 + \frac{\alpha}{3}k(u_{i+1} - u_i)^3\right)
\end{align}

Ao non ser lineal a ecuación de movemento, non é posible atopar unha solución
analítica, mais si que se pode descompoñer en modos normais:

\begin{align}
    \nonumber H =\ &\frac{1}{2}\sum_{j=1}^N \left(\frac{{p'}_j^2}{m} +m\omega_j^2q_j^2\right) \\
    \nonumber &+\frac{\alpha}{3}\sum_{j,l,m=1}^N c_{jlm}a_ja_la_m\omega_j\omega_l\omega_m
\end{align}

Tras isto, calcúlanse os coeficientes non lineais $c_{jml}$ e ponse $q_j$ en
...
\begin{align}
    \nonumber S_n &= \sum_j^N(-i)^nF\left[\sum_k A\left(e^{(i\frac{j+i)k\pi}{N+1}}-e^{i\frac{jk\pi}{N+1}}\right)\right]^n\\
    \nonumber S_n &= F\sum_j^N\left[a_ke^{i\frac{(j+i/2)k\pi}{N+1}}2\sin\left(\frac{k\pi}{2(N+1)}\right)\right]^n\\
    \nonumber S_n &= F\sum_j^N\left[a_k\frac{\omega_k}{\omega_0}e^{i\frac{(j+i/2)k\pi}{N+1}}\right]^n
\end{align}

Saltando un par de pasos intermedios, os termos non lineais serían os
seguintes, tendo en conta a repetición dos índices $jlm$:

\begin{align}
    \nonumber c_{jlm} = 2k\frac{(N+1)F}{\omega_0^3}Z(x)D(x)=\frac{k}{\omega_0^3\sqrt{2(N+1)}}Z(x)D(x)
\end{align}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{revistas/004/imaxes/mary3.png}
    \caption{
        Resultados do experimento orixinal nos que se mostra que a enerxía
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{revistas/004/imaxes/mary4.png}
    \caption{
        Nota de páxina con 2 liñas de agradecemento á ``señorita Mary Tsingou''
...
revista \textit{Physics Today}, titulado``Fermi, Pasta, Ulam and a mysterious
...
\nocite{wrong_righted}
\nocite{entr_mary}
\nocite{dama_misteriosa}
\nocite{alejandro_mata}
