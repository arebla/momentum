\Titular%
{divulgacion}%
{O signo menos que vale un premio Nobel}%
{Víctor Díaz Díaz}%
{Breve historia do descubrimento da liberdade asintótica e o nacemento da cromodinámica cuántica.}%
...
da entrega do premio Nobel a D. Gross \cite{GrossNobel}, no que lembra a
...
\subsection*{O comportamento ultravioleta das QFTs}
...
\begin{equation}
    \beta(g) \equiv \frac{\text{d}g}{\text{d} \ln \mu} = \mu
    \frac{\text{d}g}{\text{d}\mu},
\end{equation}

\noindent onde $g$ é unha determinada constante de acoplamento e $\mu$ é a
escala de enerxía \cite{Ramond}.
...
$\beta(g)$ verifica a relación \cite{Srednicki}

\begin{equation}
    \int_{g(m)}^{g(\Lambda)} \frac{\text{d}g}{\beta(g)} = \ln \frac{\Lambda}{m}.
    \label{eq:RGF}
\end{equation}

\noindent É obvio que no límite $\Lambda \to \infty$ o lado dereito da ecuación
diverxe. Para o lado esquerdo, pola súa parte, poden darse dous casos \cite{Landau1}:

\textbf{1)} Que a función $\beta$ tamén diverxa, de xeito que $g(\Lambda) \to \infty$ para
algún valor finito de enerxía e o parámetro presenta o que se coñece como
\textit{polo de Landau}. Isto volve a teoría inconsistente a altas enerxías.

\textbf{2)} Se a función $\beta$ se mantén positiva e medra de xeito máis rápido que
$\mathcal{O}(g)$, o lado esquerdo de \eqref{eq:RGF} aproximarase a un valor
finito, de xeito que

\begin{equation}
    \int_{g(m)}^{\infty} \frac{\text{d}g}{\beta(g)} = \ln
    \frac{\Lambda_\text{max}}{m}.
\end{equation}

\noindent A primeira orde, podemos aproximar este valor finito da enerxía como $
\Lambda_\text{max} \approx m e^{\beta_2 / g(m)}$, onde $\beta_2$ é unha
constante positiva, de xeito que $\Lambda_\text{max} \to \infty$ implica $g(m)
\to 0$ para calquera $m$ e a teoría é \textit{trivial}, de xeito que non hai
...
\subsection*{A liberdade asintótica}
...
máis rápido que $\mathcal{O}(g)$ ou se se volve negativa a partir dalgún valor
$g_* > g(m)$. Unha teoría tal denominaríase \textit{asintoticamente libre}. No
entanto, isto era pouco máis que unha resposta \textit{ad hoc}, e ningunha das
teorías coñecidas producía eses resultados. O propio Landau aseveraba que non
podía existir ningunha teoría que fose asintoticamente libre \cite{Landau2}.

A seguinte peza nesta historia son os experimentos de \textit{deep-inelastic
...
partículas libres puntuais de spin $1/2$ \cite{DIS1, DIS2}, o cal parecía
...
\cite{GrossNobel}. Xunto con C. Callan, conseguiron demostrar que as
observacións experimentais de DIS só podían corresponderse cunha teoría con
liberdade asintótica \cite{GrossCallan} e posteriormente, xunto con S. Coleman,
que as teorías con interaccións escalares, tipo Yukawa ou gauge abelianas (como
QED) non eran asintóticamente libres \cite{GrossColeman}. As teorías gauge non
...
\begin{equation}
    \beta(\alpha) \equiv \mu \frac{\text{d}\alpha(\mu)}{\text{d}\mu}
    = \frac{\alpha^2}{\pi} b_1 + \mathcal{O}(\alpha^3),
\end{equation}

\noindent onde $\alpha = g^2/4\pi$ e $g$ é a constante de acoplamento. O
...
foi \cite{GrossWilczek, Politzer}

\begin{equation}
    b_1 = - \left(\frac{11}{6} C_A - \frac{2}{3} \sum_R n_R T_R \right).
\end{equation}

\noindent Esta expresión é válida para unha teoría $SU(N)$ xeral, e depende de
...
\subsection*{Interpretación física da liberdade asintótica}
...
\noindent Este é un feito ben coñecido na electrostática clásica, pero que pode
...
\noindent onde $m \sim r^{-1}$ en unidades naturais. Isto é una consecuencia de
...
libres é precisamente o contrario. Colocar unha <<carga non
abeliana>> no baleiro causaría que os pares presentes neste se
...
relevancia <<numérica>>, na que o resultado pode
...
perspectiva de que <<un signo non é tan importante>>
