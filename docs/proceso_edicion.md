...
+ (**RECOMENDADO**) Usando [[Git](https://git-scm.com/)] dende a terminal. Fai
...
O seguinte punto é engadir o último no primeiro.
...
- **ARQUIVO PRINCIPAL**: Sempre imos ter un arquivo *principal* co texto do
  ...
  exacta `revistas/006/artigo_NOMEAUTOR.tex`. IMPORTANTE: A dito artigo hai que
...
ficheiro inda non existe entón fai falla crealo. En xeral soe facerse logo de
...
temporais, senón a revista non se poderá compilar. Usade un número anterior
como exemplo.
...
Unha vez poidamos compilar un PDF, podemos recompilar o documento ca opción
`simple` facendo
...
Esto xera unha versión simplificada sen portada, índice nin contraportada. Dita
versión debe entregarse aos redactores pra que dean a súa opinión.
